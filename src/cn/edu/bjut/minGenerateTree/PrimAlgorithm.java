...
public class PrimAlgorithm implements MinGenerateTreeAlgorithm {
	...
	public void buildMinGeneratedTree(MGraph g, int start) {
		...
		/*
		 * for (int i = 0; i < g.n; i++) { if (g.edges[start][i] > 0) { path[i]
		 * = start; } }
		 */
				...
				for (int j = 0; j < g.n; j++) {
					if (closed.get(i) != j) {
						if (!closed.contains(j)) {
							if (g.edges[closed.get(i)][j] > 0) {
								if (min == -1
										|| g.edges[closed.get(i)][j] < min) {
									min = g.edges[closed.get(i)][j];
									k = j;
								}
							}
						}
